...
// ▼▼▼ [수정] MainActivity로 전달할 사용자의 숫자 ID(id)를 추가합니다. ▼▼▼
...
class SplashActivity : AppCompatActivity() {
            ...
            for (attempt in 1..MAX_ATTEMPTS) {
                ...
                try {
                    ...
                    if (response.isSuccessful) {
                        // ▼▼▼ [수정] moveToMain 함수에 deviceId를 전달합니다. ▼▼▼
    ...
    private fun moveToMain(user: SplashUser?, deviceId: String?) {
        // [핵심 수정] SharedPreferences에 deviceId를 저장합니다.
        if (deviceId != null) {
        ...
        val intent = Intent(this, MainActivity::class.java).apply {
            // [핵심 수정] MainActivity에 사용자의 숫자 ID도 함께 전달합니다.
